    ...
    <header className="bg-pwc-black text-white h-14 flex items-center px-6 sticky top-0 z-50">
      <Link href="/" className="flex items-center mr-8">
        <svg width="48" height="28" viewBox="0 0 48 28" fill="none" xmlns="http://www.w3.org/2000/svg">
          <text x="0" y="22" fill="#d04a02" fontSize="18" fontWeight="bold" fontFamily="Helvetica, Arial, sans-serif">PwC</text>
        </svg>
      </Link>
      ...
      {isAuthenticated && user ? (
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <div className="w-8 h-8 rounded-full bg-pwc-orange flex items-center justify-center text-sm font-medium">
              {user.role === "admin" ? "A" : user.name.charAt(0)}
            </div>
            <span className="text-sm text-pwc-gray-300">{user.role === "admin" ? "관리자" : user.name}</span>
            <svg
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-pwc-gray-200 py-1 text-pwc-black">
              <div className="px-4 py-3 border-b border-pwc-gray-200">
                <p className="text-sm font-medium">{user.role === "admin" ? "관리자" : user.name}</p>
        ...
        <Link href="/login" className="text-sm text-pwc-gray-300 hover:text-white px-3 py-1.5 rounded hover:bg-pwc-gray-700 transition-colors">
          로그인
        </Link>
      )}
    </header>
