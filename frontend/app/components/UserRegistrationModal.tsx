...
const permissionOptions = [
  ...
  { key: "pdf_export", label: "PDF 추출" },
  { key: "excel_export", label: "Excel 추출" },
  { key: "print", label: "인쇄" },
  { key: "share", label: "공유" },
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
        ...
        <div className="sticky top-0 bg-white border-b border-pwc-gray-200 px-6 py-4 rounded-t-xl flex items-center justify-between">
          <h2 className="text-lg font-semibold text-pwc-black">
            사용자 등록
          </h2>
          <button
            onClick={onClose}
            className="text-pwc-gray-400 hover:text-pwc-gray-600 transition-colors"
          >
        ...
        <div className="px-6 py-4 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-medium text-pwc-gray-700">
                상세 권한 설정
              </label>
            ...
            <div className="grid grid-cols-2 gap-2">
              {permissionOptions.map((perm) => (
                <label
                  key={perm.key}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg border border-pwc-gray-200 hover:bg-pwc-gray-50 cursor-pointer transition-colors"
                >
                  <input
                    type="checkbox"
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                  sendWelcomeEmail ? "bg-pwc-orange" : "bg-pwc-gray-300"
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                    sendWelcomeEmail ? "translate-x-6" : "translate-x-1"
                  }`}
                />
              </button>
            </div>
              ...
              <div className="space-y-3">
                ...
                {showPreview && (
                  <div className="border border-pwc-gray-200 rounded-lg p-4 bg-pwc-gray-50">
                    <div className="text-xs text-pwc-gray-500 mb-2">
                      To: {email || "user@company.com"}
                    </div>
                    <div className="text-xs text-pwc-gray-500 mb-3">
                      Subject: Easy View 포탈 계정 초대
                    </div>
                    <div className="border-t border-pwc-gray-200 pt-3">
                      <div className="text-sm text-pwc-gray-700 whitespace-pre-line">
                        {welcomeMessage}
                      </div>
                    </div>
                  </div>
